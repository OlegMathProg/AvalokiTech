...
type
  ...
  TSelPts            =class {$region -fold}
    public
      {selection image:circle,rectangle etc.} {$region -fold}
      sel_img_var                : TFastLine;
      sel_img_big_var            : TFastLine; {$endregion}
      {outer subgraph edges}
      outer_subgraph_var         : TFastLine;
      {inner subgraph edges}
      inner_subgraph_var         : TFastLine;
      {selected points}
      selected_pts_var           : TFastLine;
      ...
      constructor Create                        (constref w,h                :integer;
                                                 ...
                                                 constref bckgd_width,
                                                          bckgd_height       :integer;
                                                 constref rct_out            :TPtRect);           {$ifdef Linux}[local];{$endif}
      {destroy class instance}
      destructor  Destroy;                                                              override; {$ifdef Linux}[local];{$endif}
      {TODO}
...
begin
  ...
  rct_eds_big_var:=TFastLine.Create;
  with rct_eds_big_var do
    begin
  ...
  rct_pts_big_var:=TFastLine.Create;
  with rct_pts_big_var do
    begin
      SplineInit(w,h,False,True,False,False);
      SetBckgd  (bckgd_ptr,bckgd_width,bckgd_height);
...
begin
  with rct_eds_var[spline_ind] do
    rct_out_:=Rect
    (
      rct_out_ptr^.left,
      rct_out_ptr^.top,
...
begin
  ...
  // spline edges bounding rectangles:
  rct_eds_var[m]           :=TFastLine.Create;
  rct_eds_var[m].local_prop:=global_prop;
  // spline points bounding rectangles:
  rct_pts_var[m]           :=TFastLine.Create;
  rct_pts_var[m].local_prop:=global_prop;
  // spline edges:
      eds_var[m]           :=TFastLine.Create;
      eds_var[m].local_prop:=global_prop;
  // spline points:
      pts_var[m]           :=TFastLine.Create;
...
{LI}
constructor TSelPts.Create(constref w,h:integer; constref bckgd_ptr:PInteger; constref bckgd_width,bckgd_height:integer; constref rct_out:TPtRect);    {$ifdef Linux}[local];{$endif} {$region -fold}
begin

  outer_subgraph_var:=TFastLine.Create;
  ...
  inner_subgraph_var:=TFastLine.Create;
  ...
  selected_pts_var:=TFastLine.Create;
  ...
  sel_img_big_var:=TFastLine.Create;
  with sel_img_big_var do
    begin
      SplineInit(w,h,False,True,False,False);
      SetBckgd  (bckgd_ptr,bckgd_width,bckgd_height);
...
begin
  with sel_img_var,local_prop do
    begin
      ...
      {Clear Buffer---} {$region -fold}
      fst_img.FilNTValueArr(sel_img_big_var.ln_arr1,sel_img_big_var.ln_arr_width); {$endregion}
...
procedure TF_MainForm.SB_Select_Texture_RegionClick(sender:TObject);
  ...
  procedure PPFloodFillRot0(constref pvt:TPtPos; var bmp_src_ptr:PInteger; constref bmp_dst_ptr:PInteger; constref rct_dst,rct_clp:TPtRect; constref bmp_dst_width:longword; constref angle:double); {$region -fold}
  var
  ...
  begin
    if (Trunc(angle) mod 360=0) then
      Exit;
    {TODO: clipping}
    GetRot(pvt,angle,c,s,v,w);
    d_width      :=bmp_dst_width-rct_dst.width;
    bmp_src_ptr_2:=Unaligned(bmp_src_ptr+rct_dst.left+rct_dst.top*bmp_dst_width);
      ...
      begin
        for x:=0 to rct_dst.width-1 do
          begin
            ...
            x_:=-dx*c+dy*s+v;
            y_:=-dy*c-dx*s+w;
            if (x_>rct_clp.left) and (x_<rct_clp.right ) and
               (y_>rct_clp.top ) and (y_<rct_clp.bottom) then
              (bmp_dst_ptr+Round(x_)+Round(y_)*bmp_dst_width)^:=clRed{bmp_src_ptr_2^};
            Inc(bmp_src_ptr_2);
          end;
  ...
  with srf_var,tex_var,sln_var do
    begin
      ...
      (
        PtPos
        (
          pts_var[sln_obj_cnt-1].rct_ent.left+eds_var[sln_obj_cnt-1].rct_ent.width >>1,
          pts_var[sln_obj_cnt-1].rct_ent.top +eds_var[sln_obj_cnt-1].rct_ent.height>>1
        ),
        srf_bmp_ptr,
        low_bmp_ptr,
        PtRct(pts_var[sln_obj_cnt-1].rct_ent),
        inn_wnd_rct,
        low_bmp.width,
        90.0
      );
  ...
  if drawing_area_enter_calc then
    begin
      ...
      if (Shift<>[ssCtrl]) then
        ...
        {Resize And Draw Circle Selection}
        if down_select_points_ptr^ then
          with srf_var,crc_sel_var,brs_sel_var do
            begin
              ...
              if ((crc_rad*crc_rad)<<2+0>=min_sqr+max_sqr) or
                ...
                Exit;}
              if (crc_rad<<1+3>=Min(srf_bmp.width,srf_bmp.height)) or
                 (crc_rad<<1+3>=000000000000000000000000000004096) then
                Exit;
              crc_rad       +=10;
              crc_rad_sqr   :=crc_rad*crc_rad;
              resize_crc_sel:=False;
