...
const
  ...
  DEFAULT_MASK_TEMPLATE_SPRITE_ICON='Pics\Map_Editor\Default_Mask_Template_Sprite_Icon0.png';
...
type
  ...
  TCurve             =class {$region -fold}
    public
      ...
      rct_eds_var            : array of TFastSpline;
      rct_eds_big_var        : TFastSpline;
      rct_eds_useless_fld_arr: T1IntrArr;
      // spline points bounding rectangles buffers:
      rct_pts_var            : array of TFastSpline;
      rct_pts_big_var        : TFastSpline;
      rct_pts_useless_fld_arr: T1IntrArr;
      // spline edges buffers:
      eds_var                : array of TFastSpline;
      eds_big_var            : TFastSpline;
      eds_useless_fld_arr    : T1IntrArr;
      // spline points buffers:
      pts_var                : array of TFastSpline;
      pts_big_var            : TFastSpline;
      ...
      {checks if spline object bounding rectangles must be shown}
      procedure VisSplineRct               (constref spline_ind      :integer);       inline; {$ifdef Linux}[local];{$endif}
      {checks if spline object edges must be shown}
      procedure VisSplineEds               (constref spline_ind      :integer);       inline; {$ifdef Linux}[local];{$endif}
      {checks if spline object points must be shown}
      procedure VisSplinePts               (constref spline_ind      :integer);       inline; {$ifdef Linux}[local];{$endif}
      {checks if spline object must be shown}
      procedure VisSplineAll0              (constref start_ind,
                                                     end_ind         :integer);       inline; {$ifdef Linux}[local];{$endif}
      {checks if spline object must be shown}
      procedure VisSplineAll1              (constref start_ind,
                                                     end_ind         :integer);       inline; {$ifdef Linux}[local];{$endif}
  end; {$endregion}
  PCurve             =^TCurve;
  ...
  TSelPts            =class {$region -fold}
    public
      {outer subgraph edges arrays container}
      outer_subgraph_var         : TFastSpline;
      {inner subgraph edges arrays container}
      inner_subgraph_var         : TFastSpline;
      {TODO}
      selected_pts_var           : TFastSpline;
      {TODO}
      ...
      {TODO}
      inner_subgraph__eds_cnt    : integer;
      {minimal index of selected object(spline)}
      sel_obj_min_ind            : integer;
      {TODO}
      is_an_abst_obj_kind_after  : boolean;
      {TODO}
      draw_outer_subgraph        : boolean;
      {TODO}
      draw_inner_subgraph        : boolean;
      {TODO}
      draw_selectd_points        : boolean;
      {TODO}
      sel_pts                    : boolean;
      {TODO}
      ...
      destructor  Destroy;                                                              override; {$ifdef Linux}[local];{$endif}
      {TODO}
      procedure SelectAllPts                    (const    pts_cnt,eds_cnt    :integer);   inline; {$ifdef Linux}[local];{$endif}
      {TODO}
      procedure SelectedPtsRectDraw             (         cnv_dst            :TCanvas;
                                                          b_rct              :TRect;
                                                          color1,color2      :TColor);    inline; {$ifdef Linux}[local];{$endif}
      {TODO}
...
var
  ...
  ini_var                    : TIniFile;
  {inner window margin}
  inn_wnd_mrg                : integer=1;
  splitter_thickness         : integer=4;
  ...
  splitters_arr              : array[0..5] of PInteger;
  ...
  with srf_var,sln_var,tex_var,srf_var,tex_var,sln_var,sel_var,crc_sel_var,pvt_var,fast_physics_var,fast_fluid_var do
    begin
     ...
     {CircleHighlight(d.x,d.y,
      ...
      for i:=0 to SE_Count_Y.Value-1 do
        Point(tex_bmp_rct_pts[0].x+Random(Trunc(tex_bmp_rct_pts[1].x-tex_bmp_rct_pts[0].x)-1)+1,
              tex_bmp_rct_pts[0].y+Random(Trunc(tex_bmp_rct_pts[1].y-tex_bmp_rct_pts[0].y)-1)+1,
              srf_bmp_ptr,
              srf_bmp.width,
              color_info,
      ...
      {Minimap-----------} {$region -fold}
      {ITScaleDown(srf_bmp_ptr,srf_bmp_ptr,low_bmp.width,low_bmp.height,6,6);} {$endregion}
...
begin
  ...
  srf_var.EventGroupsCalc(calc_arr,[0,1,2,3,4,6,8,9,16,17,18,20,21,22,28,30,31,32,37]);
  SpeedButtonRepaint;
end; {$endregion}
...
begin
  ...
  with sel_var.outer_subgraph_var do
    begin
      SetBckgd  (srf_bmp_ptr,srf_bmp.width,srf_bmp.height);
      SplineInit(            srf_bmp.width,srf_bmp.height,True,True,True,True);
      GCCArrInit;
    end;
  with sel_var.inner_subgraph_var do
    begin
      SetBckgd  (srf_bmp_ptr,srf_bmp.width,srf_bmp.height);
      SplineInit(            srf_bmp.width,srf_bmp.height,True,True,True,True);
      GCCArrInit;
...
begin
  ...
  // Get Target Render Fo OpenGL Output:
...
begin
  ...
  with obj_var,sln_var,sel_var,crc_sel_var,pvt_var do
    begin
      if (not draw_outer_subgraph) and (pvt_draw_sel_eds_off<>pvt) then
      ...
      for i:=sel_obj_min_ind to sln_obj_cnt-1 do
        if (has_sel_pts[i]<>0) then
          begin
            ...
            with rct_eds_var[i],local_prop do
            ...
            with rct_pts_var[i],local_prop do
...
begin
  ...
  with sln_var do
    begin
      {Edges}
      with eds_var[sln_obj_cnt-1],local_prop do
        if hid_ln_elim then
          begin
...
begin
  ...
  with sln_var do
    begin
      {Bounding Rectangles: Edges}
      with rct_eds_var[sln_obj_cnt-1],local_prop do
        begin
          if rct_eds_show and (not IsRct1OutOfRct2(rct_ent,rct_out_ptr^)) then
            begin
      ...
      {Bounding Rectangles: Points}
      with rct_pts_var[sln_obj_cnt-1],local_prop do
        begin
          if rct_pts_show and (not IsRct1OutOfRct2(rct_ent,rct_out_ptr^)) then
            begin
              lazy_repaint_prev:=IsRct1InRct2(rct_ent,rct_out_ptr^);
      ...
      {Points}
      with pts_var[sln_obj_cnt-1],local_prop do
        begin
          if pts_show and (not IsRct1OutOfRct2(rct_ent,rct_out_ptr^)) then
            begin
              lazy_repaint_prev:=IsRct1InRct2(rct_ent,rct_out_ptr^);
...
begin
  ...
  with obj_var,sln_var do
    begin
      eds_var_ptr            :=Unaligned(@eds_var            [0]);
...
var
  rct_eds_var_ptr: PFastSpline;
  rct_pts_var_ptr: PFastSpline;
      eds_var_ptr: PFastSpline;
      pts_var_ptr: PFastSpline;
...
begin
  ...
  with obj_var,sln_var do
    begin
      ...
      rct_eds_var_ptr:=Unaligned(@rct_eds_var[0]);
      rct_pts_var_ptr:=Unaligned(@rct_pts_var[0]);
          eds_var_ptr:=Unaligned(@    eds_var[0]);
          pts_var_ptr:=Unaligned(@    pts_var[0]);
...
begin
  with sel_var do
    begin
      if draw_outer_subgraph then
        OuterSubgraphDraw;
      if draw_inner_subgraph and (not IsRct1OutOfRct2(sel_pts_rct,inn_wnd_rct)) then
        InnerSubgraphDraw;
      if draw_selectd_points then
        SelectdPointsDraw;
    end;
end; {$endregion}
{Sel. Tools Marker: Reset Background Settings}
...
begin
  ...
  rct_eds_big_var:=TFastSpline.Create;
  with rct_eds_big_var do
    begin
  ...
  rct_pts_big_var:=TFastSpline.Create;
  with rct_pts_big_var do
    begin
      SplineInit(w,h,False,True,False,False);
      SetBckgd  (bckgd_ptr,bckgd_width,bckgd_height);
  ...
  // spline edges:
  eds_big_var:=TFastSpline.Create;
  with eds_big_var do
    begin
  ...
  // spline points:
  pts_big_var:=TFastSpline.Create;
  with pts_big_var do
    begin
      SplineInit(w,h,False,True,False,False);
      SetBckgd  (bckgd_ptr,bckgd_width,bckgd_height);
...
procedure TCurve.PrimitiveComp     (constref spline_ind       :integer; constref pmt_var_ptr,pmt_big_var_ptr:PFastSpline; pmt_bld_stl:TDrawingStyle); inline; {$ifdef Linux}[local];{$endif} {$region -fold}
begin
  with pmt_var_ptr^,fst_img do
    ...
    //nt_pix_clp_type          :=0;
      ...
      {bmp_ftimg_left          :=bmp_src_rct_clp.left  ;
      bmp_ftimg_top            :=bmp_src_rct_clp.top   ;
      bmp_ftimg_width          :=bmp_src_rct_clp.width ;
      bmp_ftimg_height         :=bmp_src_rct_clp.height;
      bmp_ftimg_right          :=bmp_src_rct_clp.right ;
      bmp_ftimg_bottom         :=bmp_src_rct_clp.bottom;}
      SetSdrType;
      ShaderType;
    end;
end; {$endregion}
...
begin
  with rct_eds_var[spline_ind],local_prop do
    begin

      {Final   Rectangle} {$region -fold}
      rct_wnd:=eds_var[spline_ind].rct_wnd; {$endregion}

      {Entire  Rectangle} {$region -fold}
      rct_ent:=eds_var[spline_ind].rct_ent; {$endregion}

      {Clipped Rectangle} {$region -fold}
      rct_vis:=eds_var[spline_ind].rct_vis; {$endregion}
...
begin
  with rct_pts_var[spline_ind],local_prop do
    begin

      {Final   Rectangle} {$region -fold}
      rct_wnd:=pts_var[spline_ind].rct_wnd; {$endregion}

      {Entire  Rectangle} {$region -fold}
      rct_ent:=pts_var[spline_ind].rct_ent; {$endregion}

      {Clipped Rectangle} {$region -fold}
      rct_vis:=pts_var[spline_ind].rct_vis; {$endregion}
...
begin
  with eds_var[spline_ind],local_prop do
    begin
...
begin
  with pts_var[spline_ind],local_prop do
    begin
...
begin
  ...
  SetLength(rct_eds_var        ,sln_obj_cnt);
  SetLength(rct_pts_var        ,sln_obj_cnt);
  SetLength(eds_var            ,sln_obj_cnt);
  SetLength(pts_var            ,sln_obj_cnt);
  ...
  // spline edges bounding rectangles:
  rct_eds_var[m]           :=TFastSpline.Create;
  rct_eds_var[m].local_prop:=global_prop;
  // spline points bounding rectangles:
  rct_pts_var[m]           :=TFastSpline.Create;
  rct_pts_var[m].local_prop:=global_prop;
  // spline edges:
      eds_var[m]           :=TFastSpline.Create;
      eds_var[m].local_prop:=global_prop;
  // spline points:
      pts_var[m]           :=TFastSpline.Create;
      pts_var[m].local_prop:=global_prop; {$endregion}
  ...
  // spline edges bounding rectangles:
  with rct_eds_var[m],local_prop do
    pts_col_inv:=SetColorInv(clRed);

  // spline pointss bounding rectangles:
  with rct_pts_var[m],local_prop do
    pts_col_inv:=SetColorInv(clBlue);

  // spline edges:
  with eds_var[m],local_prop do
    begin
  ...
  with rct_eds_var[spline_ind],local_prop do
    begin
      ...
      (
        ...
        rct_eds_big_var.ln_arr1_ptr,
        rct_eds_big_var.ln_arr_width,
        rct_eds_big_var.ln_arr_height,
        PtRct(rct_vis),
        local_prop,
        @PPFloodFillAdd
      );
  ...
  with rct_pts_var[spline_ind],local_prop do
    begin
      ...
      (
        pts_rct_width__half-pts_rct_width__odd+rct_ent.left,
        pts_rct_height_half-pts_rct_height_odd+rct_ent.top ,
        rct_pts_big_var.ln_arr1_ptr,
        rct_pts_big_var.ln_arr_width,
        rct_pts_big_var.ln_arr_height,
        PtRct(rct_vis),
        local_prop,
        @PPFloodFillAdd
      );
...
begin
  with eds_var[spline_ind],local_prop do
    begin
      ...
      hid_ln_cnt:=eds_big_var.hid_ln_cnt; {$endregion}
...
begin
  with eds_var[spline_ind],local_prop do
    begin
      ...
      bmp_dst_ptr2           :=eds_big_var.bmp_dst_ptr;
      eds_big_var.bmp_dst_ptr:=@eds_useless_fld_arr[0]; {$endregion}

      {Save and Set Color(inverted value)---------------------} {$region -fold}
      eds_col_inv2                      :=eds_big_var.local_prop.eds_col_inv;
      eds_big_var.local_prop.eds_col_inv:=1; {$endregion}
      ...
      eds_big_var.bmp_dst_ptr:=bmp_dst_ptr2; {$endregion}

      {Reset Color(inverted value)----------------------------} {$region -fold}
      eds_big_var.local_prop.eds_col_inv:=eds_col_inv2; {$endregion}
      ...
      for i:=0 to sln_obj_pts_cnt[spline_ind]-1 do
        begin
          ...
          if IsPtInRct(pt.x,pt.y,rct_out_ptr^) then
            begin
              dup_pts_arr_ptr:=Unaligned(@dup_pts_arr[pt.x+pt.y*pts_big_var.ln_arr_width]);
              ...
              if (dup_pts_arr_ptr^*dup_pts_id    ) and
                ...
                begin
                  ...
                  (
                    pt.x,
                    pt.y,
                    pts_big_var.ln_arr1_ptr,
                    pts_big_var.ln_arr_width,
                    pts_big_var.ln_arr_height,
                    PtRct(rct_vis),
                    local_prop,
                    @PPFloodFillAdd
                  );
      ...
      for i:=0 to sln_obj_pts_cnt[spline_ind]-1 do
        begin
          pt:=PtPos(Trunc(sln_pts_ptr^.x)+srf_var.world_axis_shift.x,
                    Trunc(sln_pts_ptr^.y)+srf_var.world_axis_shift.y);
          if IsPtInRct(pt.x,pt.y,rct_out_ptr^) then
            dup_pts_id.SetEqual2(dup_pts_arr[pt.x+pt.y*pts_big_var.ln_arr_width],dup_pts_id);
          Inc(sln_pts_ptr);
        end;
    end;
end; {$endregion}
        ...
        for i:=0 to sln_obj_pts_cnt[j]-1 do
          begin
            pt:=PtPos(Trunc(sln_pts_ptr^.x)+srf_var.world_axis_shift.x,
                      Trunc(sln_pts_ptr^.y)+srf_var.world_axis_shift.y);
            if IsPtInRct(pt.x,pt.y,rct_out_ptr^) then
              dup_pts_arr[pt.x+pt.y*pts_big_var.ln_arr_width].obj_ind:=j;
            Inc(sln_pts_ptr);
          end;
      end;
end; {$endregion}
      ...
      for i:=0 to sln_obj_pts_cnt[spline_ind]-1 do
        begin
          pt:=PtPos(Trunc(sln_pts_ptr^.x)+srf_var.world_axis_shift.x,
                    Trunc(sln_pts_ptr^.y)+srf_var.world_axis_shift.y);
          if IsPtInRct(pt.x,pt.y,rct_out_ptr^) then
            dup_pts_id.SetEqual1(dup_pts_arr[pt.x+pt.y*pts_big_var.ln_arr_width],Default(TPtPos2));
          Inc(sln_pts_ptr);
        end;
    end;
end; {$endregion}
...
begin
  with rct_eds_var[spline_ind],local_prop,fst_img do
...
begin
  with rct_pts_var[spline_ind],local_prop,fst_img do
...
begin
  with eds_var[spline_ind],local_prop,fst_img do
...
begin
  with pts_var[spline_ind],local_prop,fst_img do
...
begin
  with rct_eds_var[spline_ind],local_prop do
    begin
      ...
      PrimitiveComp(spline_ind,@rct_eds_var[spline_ind],@rct_eds_big_var,dsAlphablend); {$endregion}

      {Clear Buffer---} {$region -fold}
      fst_img.FilNTValueArr(rct_eds_big_var.ln_arr1,
                            rct_eds_big_var.ln_arr_width); {$endregion}
...
begin
  with rct_pts_var[spline_ind],local_prop do
    begin
      ...
      // compress edges  bounding rectangle sprite:
      PrimitiveComp(spline_ind,@rct_pts_var[spline_ind],@rct_pts_big_var,dsAlphablend); {$endregion}

      {Clear Buffer---} {$region -fold}
      fst_img.FilNTValueArr(rct_pts_big_var.ln_arr1,
                            rct_pts_big_var.ln_arr_width); {$endregion}
...
begin
  with eds_var[spline_ind],local_prop do
    begin
      ...
      PrimitiveComp(spline_ind,@eds_var[spline_ind],@eds_big_var,eds_bld_stl); {$endregion}
      ...
      {Clear Buffer---} {$region -fold}
      fst_img.FilNTValueArr(eds_big_var.ln_arr1,
                            eds_big_var.ln_arr_width); {$endregion}
...
begin
  with pts_var[spline_ind],local_prop do
    begin
      ...
      PrimitiveComp(spline_ind,@pts_var[spline_ind],@pts_big_var,pts_bld_stl); {$endregion}

      {Clear Buffer---} {$region -fold}
      fst_img.FilNTValueArr(pts_big_var.ln_arr1,
                            pts_big_var.ln_arr_width); {$endregion}
...
begin
  ...
  if (has_sel_pts[spline_ind]=0) then
    begin
      if (not eds_var[spline_ind].local_prop.hid_ln_elim) then
...
begin
  for i:=start_ind to end_ind do
    begin
      ...
      if (has_sel_pts[i]=0) then
        begin
          if (not eds_var[i].local_prop.hid_ln_elim) then
...
procedure TCurve.VisSplineRct      (constref spline_ind       :integer);                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
begin
  {// spline edges  bounding rectangles:
  with rct_eds_var[spline_ind],local_prop do
    if IsRect1OutOfRect2(obj_rct_ent,rct_out_ptr^) then
      rct_eds_show:=False;
  // spline points bounding rectangles:
  with rct_pts_var[spline_ind],local_prop do
    if IsRect1OutOfRect2(obj_rct_ent,rct_out_ptr^) then
...
procedure TCurve.VisSplineEds      (constref spline_ind       :integer);                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
var
  i: integer;
begin
  {with eds_var[spline_ind],local_prop do
...
procedure TCurve.VisSplinePts      (constref spline_ind       :integer);                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
begin
  {with pts_var[spline_ind],local_prop do
    if IsRect1OutOfRect2(obj_rct_ent,rct_out_ptr^) then
      pts_show:=False;}
end; {$endregion}
procedure TCurve.VisSplineAll0     (constref start_ind,end_ind:integer);                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
var
  i: integer;
begin
  for i:=start_ind to end_ind do
...
procedure TCurve.VisSplineAll1     (constref start_ind,end_ind:integer);                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
var
  i: integer;
begin
  for i:=start_ind to end_ind do
...
{LI}
constructor TSelPts.Create(w,h:integer);                                                                                                               {$ifdef Linux}[local];{$endif} {$region -fold}
begin

  outer_subgraph_var:=TFastSpline.Create;
  with outer_subgraph_var,local_prop do
    begin
  ...
  inner_subgraph_var:=TFastSpline.Create;
  with inner_subgraph_var,local_prop do
    begin
      //local_prop :=curve_default_prop;
      eds_col    :=clBlue;
      eds_col_inv:=SetColorInv(eds_col);
  ...
  selected_pts_var:=TFastSpline.Create;
  with selected_pts_var,local_prop do
    begin
      local_prop :=curve_default_prop;
      eds_col    :=clLime;
      eds_col_inv:=SetColorInv(eds_col);
  ...
  bucket_rct.Width         :=8;
  bucket_rct.Height        :=8;
  draw_outer_subgraph      :=True;
  draw_inner_subgraph      :=True;
  draw_selectd_points      :=True;
  is_an_abst_obj_kind_after:=True;
...
begin
  with sel_var do
    begin
      if (outer_subgraph_var.local_prop.eds_col =clBlack) then
        begin
          outer_subgraph_var.local_prop.eds_col:=$01;
          F_MainForm.CD_Select_Color.Color     :=$01;
        end;
      if (inner_subgraph_var.local_prop.eds_col =clBlack) then
        begin
          inner_subgraph_var.local_prop.eds_col:=$01;
          F_MainForm.CD_Select_Color.Color     :=$01;
        end;
      if (selected_pts_var.local_prop.eds_col   =clBlack) then
        begin
          selected_pts_var.local_prop.eds_col  :=$01;
          F_MainForm.CD_Select_Color.Color     :=$01;
        end;
    end;
end; {$endregion}
        ...
        begin
          ...
          if draw_outer_subgraph then
            OuterSubgraphToBmp(Trunc(pvt_var.pvt.X),
                               ...
                               srf_bmp_ptr,
                               inn_wnd_rct);
          if draw_inner_subgraph and (not IsRct1OutOfRct2(sel_var.sel_pts_rct,inn_wnd_rct)) then
...
begin
  ...
  with m_c_var,s_c_var do
    begin
      if sel_draw then
        begin
          selected_pts_var.bmp_dst_ptr :=srf_bmp_ptr;
          selected_pts_var.ln_arr_width:=srf_bmp_rct.Width;
...
procedure TCircSel.CircleSelectionModeDraw(x,y:integer);                                                                                       inline; {$ifdef Linux}[local];{$endif} {$region -fold}
begin
  with srf_var do
    if IsPtInRct(x,y,PtRct(inn_wnd_rct.left  +crc_rad,
      ...
      CircleC(x,y,crc_rad,srf_bmp_ptr,srf_var.inn_wnd_rct,srf_bmp.width,crc_sel_col);
end; {$endregion}
...
begin
  ...
  outer_subgraph_var.bmp_dst_ptr:=bmp_dst_ptr;
  ...
  with rct_clp do
    begin
      m1:=left  +outer_subgraph_var.grid_pt_rad;
      m2:=top   +outer_subgraph_var.grid_pt_rad;
      m3:=right -outer_subgraph_var.grid_pt_rad-1;
      m4:=bottom-outer_subgraph_var.grid_pt_rad-1;
    end;
  ...
  case outer_subgraph_var.local_prop.clp_stl of
          ...
          begin
            ...
            {for i:=0 to outer_subgraph1_eds_cnt-1 do
              begin
                ...
                outer_subgraph_var.ClippedLine1
                (
                  ...
                  PtRct(m1,m2,m3,m4),
                  Unaligned(@outer_subgraph_var.LineSME),
                  Nil,
                  Nil
                );
                outer_subgraph_var.ClippedLine1
          ...
          begin
            ...
            {for i:=0 to outer_subgraph1_eds_cnt-1 do
              begin
                pts[outer_subgraph1[i].last_point].x+=n1;
                pts[outer_subgraph1[i].last_point].y+=n2;
                outer_subgraph_var.ClippedLine2
                (
                  Trunc(pts[outer_subgraph1[i].first_point].x+srf_var.world_axis_shift.x),
                  Trunc(pts[outer_subgraph1[i].first_point].y+srf_var.world_axis_shift.y),
                  Trunc(pts[outer_subgraph1[i].last_point ].x+srf_var.world_axis_shift.x),
            ...
            for i:=0 to outer_subgraph1_eds_cnt-1 do
              begin
                ...
                if IsPtInRct(PtPosF(pts_f1.x+srf_var.world_axis_shift.x,pts_f1.y+srf_var.world_axis_shift.y),rct) and
                   IsPtInRct(PtPosF(pts_l1.x+srf_var.world_axis_shift.x,pts_l1.y+srf_var.world_axis_shift.y),rct) then
                  outer_subgraph_var.LineSMN
                  (
                ...
                if IsPtInRct(PtPosF(pts_f2.x+srf_var.world_axis_shift.x,pts_f2.y+srf_var.world_axis_shift.y),rct) and
                   IsPtInRct(PtPosF(pts_l2.x+srf_var.world_axis_shift.x,pts_l2.y+srf_var.world_axis_shift.y),rct) then
                  outer_subgraph_var.LineSMN
                  (
...
begin
  ...
  inner_subgraph_var.bmp_dst_ptr:=bmp_dst_ptr;
  ...
  with rct_clp do
    begin
      m1:=left  +inner_subgraph_var.grid_pt_rad;
      m2:=top   +inner_subgraph_var.grid_pt_rad;
      m3:=right -inner_subgraph_var.grid_pt_rad-1;
      m4:=bottom-inner_subgraph_var.grid_pt_rad-1;
    end;
  rct:=PtRct(m1,m2,m3,m4); {$endregion}
  ...
  case inner_subgraph_var.local_prop.clp_stl of
    (csClippedEdges1 ): {Clipped Edges 1(Slow  )} {$region -fold}
      if (inner_subgraph__eds_cnt>0) then
        begin
          {2 alternative records of the same code block}
          ...
          {for i:=0 to inner_subgraph__eds_cnt-1 do
            begin
              ...
              inner_subgraph_var.ClippedLine1
              (
                Trunc(pts_f3.x+srf_var.world_axis_shift.x),
                Trunc(pts_f3.y+srf_var.world_axis_shift.y),
                Trunc(pts_l3.x+srf_var.world_axis_shift.x),
                Trunc(pts_l3.y+srf_var.world_axis_shift.y),
                PtRct(m1,m2,m3,m4),
                Unaligned(@inner_subgraph_var.LineSME),
                Nil,
                Nil
              );
            end;} {$endregion}
        ...
        begin
          ...
          {for i:=0 to inner_subgraph__eds_cnt-1 do
            begin
              ...
              inner_subgraph_var.ClippedLine2
              (
                Trunc(pts_f3.x+srf_var.world_axis_shift.x),
                Trunc(pts_f3.y+srf_var.world_axis_shift.y),
                Trunc(pts_l3.x+srf_var.world_axis_shift.x),
    ...
    if (clp_stl<>csResilientEdges) then
      begin
        ...
        if eds_aa then
          begin
            ArrClear   (aa_arr1,
                        rct_clp{srf_var.inn_wnd_rct}{rct_out_ptr^},
                        ln_arr_width);
            BorderCalc1(ln_arr0,
                        aa_arr1,
                        ln_arr_width,
...
begin
  with sel_var,outer_subgraph_var,local_prop do
    begin
      with args do
        begin
          alpha:=032;
...
begin
  CD_Select_Color.Execute;
  sel_var.inner_subgraph_var.local_prop.eds_col:=CD_Select_Color.Color;
  IsObjColorAMaskColor;
  {Fill Selected Bmp}
  if exp0 then
    begin
...
begin
  CD_Select_Color.Execute;
  sel_var.selected_pts_var.local_prop.eds_col:=CD_Select_Color.Color;
  IsObjColorAMaskColor;
  {Fill Selected Bmp}
  if exp0 then
    begin
...
begin
  with srf_var,sel_var,pvt_var do
    begin
      draw_outer_subgraph:=CB_Select_Points_Show_Selected_Edges.Checked;
      if draw_outer_subgraph then
...
procedure TF_MainForm.SB_Select_Texture_RegionClick(sender:TObject);
  ...
  procedure PPFloodFillRot0(constref pvt:TPtPos; var bmp_src_ptr:PInteger; constref bmp_dst_ptr:PInteger; constref rct_dst,rct_clp:TPtRect; constref bmp_dst_width:longword; constref col:integer; constref angle:double); {$region -fold}
  var
    bmp_src_ptr_2: PInteger;
    c,s,v,w,x_,y_: double;
    x,y,dx,dy    : integer;
    d_width      : integer;
  begin
    c            :=cos(angle);
    ...
    bmp_src_ptr_2:=@bmp_src_ptr[rct_dst.left+rct_dst.top*bmp_dst_width];
      ...
      begin
        for x:=0 to rct_dst.width-1 do
          begin
            ...
            x_:=dx*(1-c)+dy*s+v;
            y_:=dy*(1-c)-dx*s+w;
            if (x_>rct_clp.left) and (x_<rct_clp.right -1) and
               (y_>rct_clp.top ) and (y_<rct_clp.bottom-1) then
              bmp_dst_ptr[Trunc(x_)+Trunc(y_)*bmp_dst_width]:=bmp_src_ptr_2^;
            Inc(bmp_src_ptr_2);
          end;
        Inc(bmp_src_ptr_2,d_width);
      end;
  end; {$endregion}
    ...
    begin
      ...
      {if LineSI(sln_pts[0].x,
        ...
        begin
          ...
          Point (Trunc(pt2.x)+world_axis_shift.x,
                 Trunc(pt2.y)+world_axis_shift.y,
                 srf_bmp_ptr,
                 srf_bmp.width,
                 col_info,
                 inn_wnd_rct);
          Circle(Trunc(pt2.x)+world_axis_shift.x,
                 Trunc(pt2.y)+world_axis_shift.y,
                 10,
                 srf_bmp_ptr,
                 srf_bmp.width,
                 col_info);
          pt4:=CrPosF(pt2.x,pt2.y,10);
          pt3:=LineCircIntPt
          (
            sln_pts[0].x,
            sln_pts[0].y,
          ...
          Point (Trunc(pt3.x0)+world_axis_shift.x,
                 Trunc(pt3.y0)+world_axis_shift.y,
                 srf_bmp_ptr,
                 srf_bmp.width,
                 col_info,
                 inn_wnd_rct);
          Circle(Trunc(pt3.x0)+world_axis_shift.x,
                 Trunc(pt3.y0)+world_axis_shift.y,
                 10,
                 srf_bmp_ptr,
                 srf_bmp.width,
                 col_info);
          Point (Trunc(pt3.x1)+world_axis_shift.x,
                 Trunc(pt3.y1)+world_axis_shift.y,
                 srf_bmp_ptr,
                 srf_bmp.width,
                 col_info,
                 inn_wnd_rct);
          Circle(Trunc(pt3.x1)+world_axis_shift.x,
                 Trunc(pt3.y1)+world_axis_shift.y,
                 10,
                 srf_bmp_ptr,
                 srf_bmp.width,
      ...
      LineS (sln_pts[0].x,
             ...
             dir_x,
             dir_y);
      Point (Trunc(pt5.x)+world_axis_shift.x,
             Trunc(pt5.y)+world_axis_shift.y,
             srf_bmp_ptr,
             srf_bmp.width,
             col_info,
             inn_wnd_rct);}

      ArrClear   (sel_var.outer_subgraph_var.aa_arr1,
                  inn_wnd_rct,
                  srf_bmp.width);
      BorderCalc0(srf_bmp_ptr,
                  sel_var.outer_subgraph_var.aa_arr1,
                  srf_bmp.width,
                  srf_bmp.width,
                  inn_wnd_rct,
                  sel_var.outer_subgraph_var.aa_nz_arr_items_count,
                  clBlack);
      BorderBlur (srf_bmp_ptr,
                  sel_var.outer_subgraph_var.aa_arr1,
                  srf_bmp.width,
                  srf_bmp.width,
                  inn_wnd_rct,
      ...
      {ArrFillTest(sel_var.outer_subgraph_var.aa_arr1,
                  srf_bmp_ptr,
                  srf_bmp.width,
                  srf_bmp.height,
                  inn_wnd_rct,
      ...
      {ArrClear(arr,inn_wnd_rct,low_bmp.width,0);}
      {PPFloodFillRot1
      (
        PtPos
        (
      ...
      LowerBmpToMainBmp;
      //BlurEmpty(@arr[0],srf_bmp_ptr,low_bmp.width,low_bmp.height);
      CnvToCnv(srf_bmp_rct,Canvas,srf_bmp.Canvas,SRCCOPY);
      InvalidateInnerWindow;}
      {LowerBmpToMainBmp;
      ITScaleDown(low_bmp_ptr,srf_bmp_ptr,low_bmp.width,low_bmp.height,2,2);
      CnvToCnv(srf_bmp_rct,Canvas,srf_bmp.Canvas,SRCCOPY);
      InvalidateInnerWindow;}
      {SetColorInfo(clRed,col_info);
      LineA(10,10,670,80,srf_bmp_ptr,srf_bmp.width,col_info);
      CnvToCnv(srf_bmp_rct,Canvas,srf_bmp.Canvas,SRCCOPY);
      InvalidateInnerWindow;}
...
begin
  ...
  with srf_var do
    tilemap_sprite_icon:=TFastImage.Create(srf_bmp_ptr,
                                           ...
                                           inn_wnd_rct,
                                           max_sprite_w_h_rct,
                                           Application.Location+DEFAULT_MASK_TEMPLATE_SPRITE_ICON,
...
begin
  with srf_var,tlm_var do
    begin
      ...
      tilemap_arr1[High(tilemap_arr1)].LoadFromFile(F_MainForm.OPD_Add_Mask_Template.Filename);
...
begin
  SplittersPosCalc;
  srf_var.EventGroupsCalc(calc_arr,[0,1,2,3,4,16,17,18,21,22,30,31,32,37]);
  with srf_var do
    begin
  ...
  if down_select_points_ptr^ then
    begin
      ...
      if crc_sel_var.draw_crc_sel then
        with srf_var,sln_var,crc_sel_var,brs_sel_var do
          begin
            need_repaint:=True;
            if exp0 then
              LowerBmp2ToMainBmp
                ...
                0,1:
                  with srf_var do
                    begin
                      ...
                      LowerBmpToMainBmp;
                      ResizeCircleSelectionModeDraw;
                      CircleSelectionModeDraw(crc_sel_rct.left+crc_sel_rct.width >>1,
                                              crc_sel_rct.top +crc_sel_rct.height>>1);
                      CnvToCnv(srf_bmp_rct,Canvas,srf_bmp.Canvas,SRCCOPY);
                      need_repaint      :=False;
...
begin
  ...
  for i:=0 to SB_Image_List.ControlCount-1 do
    SB_Image_List.Controls[i].Anchors:=[akLeft,akTop,akRight,akBottom];
  ...
  P_AnimK_Custom_Panel.Visible  :=False;
  ...
  {Main Layer-----------} {$region -fold}
  srf_var:=TSurf.Create(F_MainForm.width,
                        F_MainForm.height); {$endregion}

  {Texture--------------} {$region -fold}
  tex_var:=TTex.Create(512,512); {$endregion}

  {Snap Grid------------} {$region -fold}
  sgr_var       :=TSGrid.Create(F_MainForm.width,
                                F_MainForm.height);
  down_sgrid_ptr:=Unaligned(@SB_SGrid.Down); {$endregion}

  {Grid-----------------} {$region -fold}
  rgr_var             :=TRGrid.Create(F_MainForm.width,
                                      F_MainForm.height);
  ...
  sel_var               :=TSelPts.Create(F_MainForm.width,
                                         F_MainForm.height);
